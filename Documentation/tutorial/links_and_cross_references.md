...
The unique identifier of a file in DocFX is called UID, which stands for unique identifier. For a Markdown file, you can specify its UID by adding a UID metadata in the [YAML header](../spec/docfx_flavored_markdown.md#yaml-header). For example, the following Markdown defines a UID "fileA".
